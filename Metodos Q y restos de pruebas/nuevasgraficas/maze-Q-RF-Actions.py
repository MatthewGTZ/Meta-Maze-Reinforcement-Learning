...
def get_action(state, e, surroundings, actions_queue):
    ...
    if random.random() <=e:
        ...
        if actions_queue.qsize() == 5:
            ...
            input = scaler.transform(np.array([int(actions, base=4)]).reshape(-1, 1))
...
def create_model(file):
    ...
    for sequence in loaded_array:
        ...
        previous_actions.append(int(experience_str, base=4))
        target_actions.append(sequence[5][1])

    X = scaler.fit_transform(np.array(previous_actions).reshape(-1, 1))
