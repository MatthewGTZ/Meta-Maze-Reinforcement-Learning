...
def get_action(state, e, surroundings, actions_queue, surroundings_queue):
    ...
    if random.random() <=e:
        ...
        if actions_queue.qsize() == 5:
            surround = [elem for elem in list(surroundings_queue.queue)]
            actions = [elem for elem in list(actions_queue.queue)]
            X = np.stack((surround, actions), axis=1)
            X = X.reshape(-1, 2)
            X = scaler.transform(X)
            X = X.reshape(-1, 5, 2)
            input = torch.Tensor(X).view([-1,2])
            _, sample = model.forward(input)
            action = int(torch.argmax(sample[4]))
...
file = "lstm_scaler_surroundings_actions.pkl"
scaler = load_scaler(file)
file = "lstm_model_surroundings_actions.pkl"
